...
const isValidQuestionData = questionData => {
    return questionData.question_name && questionData.question_link && questionData.question_link.length > 0;
};

export const addQuestion = async (req, res, next) => {
    const { cid } = req.params;
    if (!isValidID(cid)) {
        return next('Invalid Category ID');
    }
    try {
...
const addMultipleQuestions = async (req, res, category, allQuestions, next) => {
    ...
    for (const questionData of req.body) {
        ...
        const existingQuestion = allQuestions.find(que => que.question_name === questionData.question_name);
        if (existingQuestion) {
    ...
    if (duplicateQuestions.length > 0) {
        return res.status(400).json({ message: 'Duplicate Question names are found', duplicateQuestions });
    }
    ...
    try {
        ...
        await category.save();
        return res.status(201).json({ message: 'Questions added successfully', addedQuestions });
    } catch (error) {
        return next(error);
    }
};
...
const addSingleQuestion = async (req, res, category, allQuestions, next) => {
    if (!isValidQuestionData(req.body)) {
        return next('Question name and question_link are required');
    }

    const existingQuestion = allQuestions.find(que => que.question_name === req.body.question_name);
    if (existingQuestion) {
        return next('Question with the same name already exists');
    }
...
export const getAllQuestions = async (req, res, next) => {
    ...
    try {
        const queryResult = await questionsmodel.find(queryObject);
        if (!difficulty) {
