    ...
    try {
        if (Array.isArray(req.body)) {
            ...
            for (const categoryData of req.body) {
                if (!categoryData.category_name || !categoryData.category_resources || categoryData.category_resources.length === 0) {
                    next('Category name and resources are required');
                    return;
                }

                if (allCategories.some(cat => cat.category_name === categoryData.category_name)) {
            ...
            if (duplicateCategoryNames.length > 0) {
                res.status(400).json({ message: 'Duplicate category names found', duplicateCategoryNames });
            } else {
                const addedCategories = await categorymodel.create(categoriesToAdd);
                res.status(200).json({ message: 'Categories added successfully', addedCategories });
            }
        }
        else if (typeof req.body === 'object') {
            if (!req.body.category_name || !req.body.category_resources || req.body.category_resources.length === 0) {
                next('Category name and resources are required');
                return;
            }
            try {
