...
export const showStatusQuery = async (req, res, next) => {
    ...
    try {
        ...
        if (status==='Revisit'||status==='Completed') {
            queryResults = await responsemodel.find({ CreatedBy: User_info, Question_Status: status });
        } 
        else if(status==='Pending'||!status){
            const allQuestions = await questionsmodel.find({});
            const userResponses = await responsemodel.find({ CreatedBy: User_info });

            const pendingQuestions = allQuestions.filter(question => !userResponses.some(response => response.Question_id.equals(question._id)));

            if(!status){
                const RemainingQuestions=await responsemodel.find({ CreatedBy: User_info });
                queryResults=[...pendingQuestions,...RemainingQuestions]
            }
            else{
                const responseQuestions = userResponses.filter(response => !response.Question_Status || response.Question_Status === 'Pending');
